	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float4 Albedo84 = CalculateContrast(_Brightness,float4( desaturateVar211 , 0.0 ));
				...
				float fresnelNode163 = ( 0.0 + 1.0 * pow( max( 1.0 - fresnelNdotV163 , 0.0001 ), _PowerFresnel ) );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float fresnelNode163 = ( 0.0 + 1.0 * pow( max( 1.0 - fresnelNdotV163 , 0.0001 ), _PowerFresnel ) );
				float4 temp_output_465_0 = ( _ColorFresnel1 * fresnelNode163 * _FresnelStrength );
				float2 uv_BumpMap = IN.ase_texcoord3.xyz.xy * _BumpMap_ST.xy + _BumpMap_ST.zw;
				float3 unpack2 = UnpackNormalScale( tex2D( _BumpMap, uv_BumpMap ), _BumpScale );
				unpack2.z = lerp( 1, unpack2.z, saturate(_BumpScale) );
				float3 tex2DNode2 = unpack2;
				float3 NormalmapXYZ170 = tex2DNode2;
